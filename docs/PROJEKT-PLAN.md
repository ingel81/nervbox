...
| `nervbox-mixer` | `main` | Mixer UI (Angular 20) - eigenständig, Sibling-Repo |
...
├── NervboxDeamon                   # .NET 8 Backend (self-contained)
├── appsettings.json
├── wwwroot/
│   ├── index.html                  # Player UI (Angular 21)
│   ├── main.*.js
│   ├── styles.*.css
│   └── mixer/                      # Mixer UI (Angular 20)
│       ├── index.html
│       ├── main.*.js
│       └── styles.*.css

/opt/nervbox-data/                  # Persistente Daten (nicht bei Deployment überschrieben)
├── nervbox.db                      # SQLite Datenbank
